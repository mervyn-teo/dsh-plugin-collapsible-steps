window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // For an assistant-step, collapse only its "Think" (reasoning) disclosure,
    // leaving the answer text visible. Other card kinds collapse the whole row.
    function stepTarget(flowItem) {
      if (flowItem.getAttribute("data-chat-flow-kind") === "assistant-step") {
        return flowItem.querySelector('[data-variant="think"]');
      }
      return flowItem;
    }
    ...
    function CollapsibleStepsCard(props) {
      var snap = props.useCollapsibleStepsCard(function (s) { return s; });
      var open = React.useState(false);
      if (!snap || snap.status !== "ready") return null;
      var value = snap.value && typeof snap.value.collapseByDefault === "boolean" ? snap.value.collapseByDefault : true;
        ...
        open[0] ? React.createElement("div", { className: "dscs-settings-body" },
          React.createElement("label", { className: "dscs-settings-row" },
            React.createElement("input", {
              ...
              checked: value,
              disabled: !writable,
              onChange: function (e) { props.toggleDefault(e.target.checked); },
            }),
    ...
    function apply(ctx) {
      ...
      // Settings preference + card. The settings scope is optional: when it is
      // unavailable the collapse still works with the in-memory default.
      ctx.slots.inject("settings.plugin.item", function () {
        var settingsScope = ctx.get("settingsScope");
        if (settingsScope === undefined) return function () {};
        var scope = settingsScope.bind({ namespace: "collapsible-steps" });
        var syncDefault = function () {
          var snap = scope.getSnapshot();
          if (snap && snap.status === "ready" && typeof snap.value.collapseByDefault === "boolean") {
            defaultCollapsed = snap.value.collapseByDefault;
            scheduleSync();
          }
        };
        scope.subscribe(syncDefault);
        syncDefault();
        return ctx.slots.register(
          {
            ...
            order: 100,
            label: "Collapsible steps",
            inject: function () {
              return {
                hooks: { collapsibleStepsCard: scope },
                toggleDefault: function (value) { return scope.set("collapseByDefault", value); },
              };
            },
          },
